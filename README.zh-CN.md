...
- 扩展入口：[src/extension.activate()](src/extension.ts)
- 主命令：[src/commands/generate.registerGenerateCommand()](src/commands/generate.ts)
- 模型同步：[src/commands/syncModels.registerSyncModelsCommand()](src/commands/syncModels.ts)
- HTTP 客户端（undici + Pool + SSE）：[src/net/httpClient.HttpClient](src/net/httpClient.ts)
- 限流（并发池 + 令牌桶）：[src/net/rateLimiter.TokenBucketPool](src/net/rateLimiter.ts)
- 退避重试：[src/net/backoff.exponentialJitter()](src/net/backoff.ts)
- 模板渲染：[src/prompt/template.render()](src/prompt/template.ts)
- 编辑插入：[src/edit/insert.applyInsertions()](src/edit/insert.ts)
- 配置 Schema：[src/config/schema.ts](src/config/schema.ts)
- 日志与状态栏：[src/log/logger.ts](src/log/logger.ts), [src/ui/status.ts](src/ui/status.ts), [src/ui/progress.ts](src/ui/progress.ts)
...
所有配置均在 [package.json](package.json) 的 contributes.configuration 中声明，并在 [src/config/schema.ts](src/config/schema.ts) 定义了类型与默认值。常用项：
...
- 请求参数
  ...
  - multiCursorAI.reasoningEffort: "none" | "low" | "medium" | "high"（默认 "medium"）
  - multiCursorAI.disableReasoning: 禁用推理（针对支持推理的模型）（默认 false）
  - multiCursorAI.singleLineOutput: 强制单行输出（默认 false）
  - multiCursorAI.promptHistoryLimit: 提示词历史记录上限（默认 100）
...
配置读取逻辑见 [src/config/index.getEffectiveConfig()](src/config/index.ts)，默认会从 SecretStorage 读取 API Key，若为空则尝试从环境变量注入并保存。
...
- 设置 API Key：multiCursorAI.setApiKey
- 清除 API Key：multiCursorAI.clearApiKey
- 切换流式输出：multiCursorAI.toggleUseStreaming
- 设置温度：multiCursorAI.setTemperature
- 设置前置分隔符：multiCursorAI.setPreSeparator
- 切换结果修剪：multiCursorAI.toggleTrimResult
- 设置日志级别：multiCursorAI.setLogLevel
- 设置推理强度：multiCursorAI.setReasoningEffort
- 切换禁用推理：multiCursorAI.toggleDisableReasoning
- 切换单行输出：multiCursorAI.toggleSingleLineOutput
...
实现参考：
- 生成命令：[src/commands/generate.registerGenerateCommand()](src/commands/generate.ts)
- 插入逻辑：[src/edit/insert.applyInsertions()](src/edit/insert.ts), [src/edit/insert.StreamInserter](src/edit/insert.ts)
...
- HTTP 客户端实现见 [src/net/httpClient.HttpClient](src/net/httpClient.ts)。

## 限流与重试

- 限流器：[src/net/rateLimiter.TokenBucketPool](src/net/rateLimiter.ts)
  - 最大并发 + 每分钟令牌桶
  - 动态探测：基于响应头（Retry-After、x-ratelimit-*）自动调整
  - cancelAll 支持全局取消队列任务
- 重试：指数退避 + 抖动
  - 算法实现：[src/net/backoff.exponentialJitter()](src/net/backoff.ts)

## 日志与状态栏

- OutputChannel 日志：[src/log/logger.Logger](src/log/logger.ts)
- 状态栏吞吐显示：[src/ui/status.StatusBarController](src/ui/status.ts)
- 进度通知（可取消）：[src/ui/progress.withCancellableProgress()](src/ui/progress.ts)
...
- 429/限流：已内置退避重试与动态限流，可降低并发或提高限额。详见 [src/net/rateLimiter.ts](src/net/rateLimiter.ts)。
...
测试文件：
- 退避重试：[test/backoff.test.ts](test/backoff.test.ts)
- 限流池：[test/rateLimiter.test.ts](test/rateLimiter.test.ts)
- 模板渲染：[test/template.test.ts](test/template.test.ts)
